...
| Balamb Garden card opponents | “Who to play for common cards” used broad level-by-level exceptions and did not distinguish recurring opponents from random hallway appearances. | Replaced it with a location-based roster and reported card-level pools from the [GameFAQs Garden card-player survey](https://gamefaqs.gamespot.com/boards/197343-final-fantasy-viii/53781094). The list now separates regular players and random spawns, notes the 2F conversation timing, and retains the Level 5 farming tip with its observational caveat. |
...
| Disc 1 | Preparing for the Exam | Reviewed card opportunities, sidequest timing, GF learning, enemy entries, and refinement paths. Consolidated the repeated Fastitocalon Water Crystal route and updated common-card targets with the Garden player-stock roster. |
