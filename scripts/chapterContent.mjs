...
export const CHAPTER_CONTENT = {
  ...
  'd1-preparing-for-the-exam': [
    `After the opening cinematic, Squall wakes in the Garden infirmary. Dr. Kadowaki asks a question — answer freely, the choice affects only dialogue. Name Squall when prompted, then follow Quistis down the hall.`,

    `Squall begins at **Lv 7** with the Revolver gunblade. Starting inventory: **5,000 Gil** · **6× Potion** · **4× Phoenix Down** · **2× Remedy** · **5× Tent**.`,

    `Once back in the classroom, **examine the desk in front of Squall** and choose "Turn on the power and…", then navigate to **Tutorial**. This awards **Quezacotl** (GF 1/16) and **Shiva** (GF 2/16) — the first two Guardian Forces. Back out with Triangle, then speak with Quistis. She confirms the study panel is checked and will wait at the Front Gate.`,

    `Walk down the second-floor hallway until someone bumps into Squall. Choose "Are you okay?" then "Sure." to agree to give her a tour of Balamb Garden. In the elevator hallway, **talk to the guy in front of the elevator** — he hands over **7 cards**: Geezard, Funguar, Red Bat, Gayla, Gesper, Fastitocalon-F, and Caterchipillar. Take the elevator down.`,

    `In the **hallways between floors**, a boy runs back and forth — challenge and beat him to win the **MiniMog Card**. This card is needed for the Queen of Cards quest on Disc 1. **Zell's Love Quest (Step 1)**: Before heading to the Front Gate, visit the **Library** and speak to the **Library Girl with the pigtail** — she explains the library rules. This starts a multi-step sidequest that eventually rewards **Combat King 003**. Do this before leaving for the Fire Cavern or she won't be around; also collect the **Occult Fan I** (1/4) from the bookshelf in the library. In the **Cafeteria**, challenge one of the Trepie fans at the tables to Triple Triad to win the **Quistis Card** — one of the most powerful level 9 cards in the game (Card Mods into **3× Samantha Soul**, each of which refines into **60× Triple** via Time Mag-RF, for a total of 180× Triple — among the best junction spells for Spd-J and St-Def-J). The **Esuna draw point** is in the Library, but a GF must be junctioned before it becomes accessible — return once junctions are set. Also in the Cafeteria, seek out the **Cafeteria Lady** (the woman behind the counter who delivers lengthy monologues) and talk to her repeatedly until she has no new lines — exhaust her entire pool of unique dialog. This interaction window becomes permanently unavailable at a later point in the game, so visiting her now ensures nothing is missed.`,

    `**Novice SeeD Candidates quest** (5-part missable sidequest): Three students in the Balamb Garden Cafeteria are trying to pass the SeeD exam. Speak to them at five specific time windows for a bonus scene upon completing all conversations. Window 1: **Now, before the SeeD Exam** — visit the Cafeteria before leaving for the Fire Cavern and speak to all three novice students at the cafeteria tables. Window 2: After the exam, before departing for Timber. Window 3: After the D-District Prison escape, before arriving at Fisherman's Horizon. Window 4: Disc 2, after the Garden Festival at Fisherman's Horizon. Window 5: Disc 3, before the Trabia Canyon dream sequence. Completing all five windows rewards a unique graduation scene. Miss any window and the quest cannot be completed.

Balamb Garden uses the **Lobby Hall** as its central hub. Branching outward: the **Cafeteria** (northwest), **Dormitory** (north), **Parking Lot** (northeast), **Training Center** (east), **Library** (southeast), **Front Gate** (south), **Infirmary** (southwest), and **Quad** (west).`,

    `Head to the **Front Gate** where Quistis delivers the Junction Tutorial. After the tutorial, exit to the world map. The stone road leading east is a safe zone — no random encounters as long as Squall stays on it. The **Fire Cavern** lies to the east; the forest between the road and the cavern produces encounters.`,

    `{{ENCOUNTERS:0}}`,
    `In the forest between Balamb Garden and the Fire Cavern, draw and stock 100× each of **Fire** (from Bite Bugs), **Blizzard** (from Glacial Eyes), and **Thunder** + **Cure** (from Caterchipillars) before entering. At the low levels of this chapter (Lv7), these are the dominant draws available — they are the most efficient early junction fillers for Str-J, Mag-J, and HP-J. **Squall's attacks cannot deal critical hits** — unlike all other party members, Squall's gunblade never rolls a critical multiplier. His damage is consistent but has no critical ceiling; what looks like a "crit" window in Renzokuken is the R1 timing bonus (+50%), not a true critical. Pressing **R1** at the right moment during Squall's Attack swing deals +50% damage per trigger — the correct timing is to wait until you hear the sword swipe sound, then immediately press R1 (pressing too early misses the window). The same R1 trigger applies during Renzokuken: pressing R1 at the right moment on each hit of the barrage adds 50% damage to that specific hit. The trigger window can be set to Automatic in Squall's Status Screen to let the CPU handle it, but manual triggering is more reliable. **Renzokuken hit count and finisher probabilities by Crisis Level**: CL1 = 4–5 hits, 23.83% finisher trigger chance; CL2 = 5–6 hits, 47.26%; CL3 = 6–7 hits, 70.70%; CL4 = 7–8 hits, 94.14%. Finisher base power values: Rough Divide = 48, Blasting Zone = 44, Fated Circle = 38. **Lion Heart delivers 17 hits** at base power 100 per hit — by far the highest finisher damage, especially at high Strength. When a finishing blow triggers at the end of Renzokuken, the exact move used depends on which weapons have been unlocked: with only Revolver the finishing blow is always Rough Divide; with Revolver+Shear/Cutting Trigger it is 25% Rough Divide / 75% Fated Circle; with all weapons except Lionheart it is 25% each of Rough Divide / Fated Circle and 50% Blasting Zone; with Lionheart it is 25% each of all four finishing moves. These percentages are fixed and cannot be changed — the only way to increase the chance of Lion Heart specifically is to maximize the chance of a finishing blow triggering at all, which requires Squall at ≤10% max HP under Aura status. Note: Fated Circle and Blasting Zone hit all enemies (AoE); avoid using them against Adel as Rinoa is attached to Adel and AoE attacks will hit Rinoa. In **Balamb Town** (southwest of Garden), challenge local students to spread the **Diff** rule from Balamb Garden into the regional card pool. **Trading rule priority**: The trading rule hierarchy degenerates in one fixed direction only: **All → Direct → Diff → One** — rules never cycle backward. Always aim to keep **Diff** (take cards based on score difference: win by 1 game-point = take 2 cards; win by 3+ = take 4 cards; win by 5+ = take all 5 rival cards). Avoid Direct (opponent keeps any of your cards they flipped) and avoid letting All degenerate into Direct. If the trading rule is not Diff, play a game with the Queen of Cards in the region you want to change; her games randomly shift the local trade rule, and you can reset until Diff is achieved. **Play rule mechanics reference**: **Same** — place a card adjacent to one or more existing cards; if the touching edge values are all equal, the matching cards flip to your color. **Plus** — if the sum of two touching edges equals the same value on at least two separate sides, those cards flip. **Combo** — any card flipped by Same or Plus immediately applies normal win comparisons to all its adjacent cards; those can then trigger further Combos, creating chain reactions. Combos can flip a large portion of the board from a single well-placed card in a Same/Plus region — this makes Same and Plus far more powerful when Combo is also active. **Elemental** — each board tile displays a colored symbol (or blank); a card placed on a matching-element tile gets +1 added to all its edge values (making it easier to win comparisons); a non-matching element tile deals −1 to all edge values (making it easier for the opponent to flip). Know your card elements before playing in Elemental regions. **Same Wall** — the board edges count as a value of "A" (10); a card touching the wall with the same value as the wall (i.e., a 10 on that edge) contributes to a Same trigger. **Paying the Queen of Cards 30,000 Gil** at any time spreads one play rule into her current region — she always introduces the first rule that area is missing, following this fixed order: Open → Same → Plus → Random → Sudden Death → Same Wall → Elemental. **How rule-mixing works (general mechanic)**: to carry rules from Region A into Region B, challenge opponents in Region A until they stop proposing to mix rules — once they stop, you are "carrying" that region's active ruleset. Travel to Region B and initiate a card game with any player there. You do not need to complete the game: accepting the match screen and immediately quitting still counts as a play event and can trigger a rule change (abolish or spread). Reset and retry if the wrong rule changes. To avoid accidentally spreading unwanted rules, decline every mixing proposal until the opponent stops asking — after that they will not propose again and you can play safely. **Two trade-rule patterns**: The game alternates between two change behaviors. Pattern 1 ("conservative"): any trade rule change immediately jumps all the way to One. Pattern 2 ("spreading"): the rule steps down one position on the ladder (All→Direct, Direct→Diff, Diff→One). To check which pattern is active, speak to the Queen of Cards and ask about the trade rule — "People are conservative" means Pattern 1 (avoid trying to change rules; results always go to One); "There is some trading going on" means Pattern 2 (step-by-step changes are possible). **Direct rule danger**: Under Direct, at game end every card in the opponent's color goes to them and every card in your color goes to you — there is no selection. Losing a narrow game (e.g., 6-4 where 4 of your own cards were flipped) means permanently losing those 4 cards to the opponent with no choice. Never let the Direct trade rule dominate a region where rare cards are present. **Card name colors**: In the card selection screen at the start of a game, **white card names** are cards you own; **blue names** are cards you have never collected; **yellow names** are cards you once owned but have lost all copies of. This color system helps track collection gaps. **110-card completion marker**: Once you have possessed all 110 unique card types at least once (even if some were later lost or modded), a **small yellow star** appears next to the "Cards" heading in the main menu. This is a permanent flag that tracks whether every card has passed through your inventory at any point — it does not require holding all 110 simultaneously. Collecting then Card Modding a rare card still counts toward this flag as long as you owned it at some point during the playthrough. **Regional starting rules** (default play rules per area): Balamb: Open; Galbadia: Same; Dollet: Random + Elemental; Fisherman's Horizon: Elemental + Sudden Death; Centra: Same + Plus + Random; Trabia: Random + Plus; Esthar: Elemental; Lunar Base/Ragnarok: all rules active. **Trade rule dominance**: Each region keeps an internal counter (0–10) tracking how many consecutive games have been played against a visitor from another region. Once 11 games with the same visitor region's rules occur, that region's trade rule fully dominates and overwrites the local rule. This is how undesirable rules (like Random or All) spread — play fewer games in a region if you want to avoid rule spread. **Recommended Balamb Garden opponents by card level**: **Trepe Groupie #2** (Cafeteria) carries the most accessible selection of **Level 3 cards** at this stage. For **Level 4 cards**, the student who sporadically appears in the hallway leading toward the Library is the strongest available source. For **Level 5 cards**, **Trepe Groupie #1** (also in the Cafeteria) is the best opponent. The **girl in the Library reading room** plays all card levels from 1 through 5, making her a reliable all-purpose opponent for filling lower-tier collection gaps. Nobody currently accessible in Balamb Garden plays **Level 6 or Level 7 cards** — those require advancing to Galbadia or other regions.`,
    `{{ENCOUNTERS:1}}`,
    `**Balamb Beach AP farming**: The beach south of Balamb Town spawns **Fastitocalon-F** enemies in pairs, awarding **6 AP per fight** with essentially no EXP. This is the best early grinding spot before the SeeD exam. Junction GFs to both Squall and Quistis and clear several fights to accelerate ability unlocking.`,
    `{{ENCOUNTERS:2}}`,

    `**Quistis Blue Magic — Disc 1 opportunities**: Before heading to the Fire Cavern, set up Quistis's Blue Magic teach items while still in Balamb. Collect **4× Malboro Cards** from Trepie fans in the Cafeteria — Card Mod them into **4× Malboro Tentacle** (Quezacotl Card Mod), then use one on Quistis to teach **Bad Breath** (at CL1: Poison, Darkness, Silence, Sleep, Slow, Confuse; at CL2: adds Berserk, Stop, Slow Petrify; at CL3: also adds Petrify and Zombie; at CL4: also adds Vit 0, Doom, and Death — one of the most powerful Blue Magics in the game). **Gesper Cards** → Card Mod → **Black Hole** → use on Quistis to teach **Degenerator** (instant KO vs non-boss; effective against most random encounters). **Gayla Cards** → Card Mod → **Mystery Fluid** — use on Quistis to teach **Acid** Blue Magic (status damage to all enemies — CL3 scaling adds **Vit 0**, making it effectively a free Meltdown plus three ailments on all enemies simultaneously), OR refine via Siren **ST Mag-RF** → **10× Meltdown** per card; Meltdown zeros enemy Vit and Spr, making every physical hit deal maximum damage and is the single most important junction-enabling spell in the game. Note: **LV? Death** is taught by **Curse Spike** — drop from Tri-Face enemies (first available in quantity at the Battle of the Gardens on Disc 2); collect them then. **Coral Fragment** (mugged from Creeps in the Timber/Galbadia forest areas, available after the SeeD exam) → use on Quistis to teach **Electrocute** Blue Magic (Thunder-elemental damage to all enemies). All these abilities can be granted regardless of whether Quistis is in the active party — teach them whenever the materials are in hand.`,

    `Enter the Fire Cavern. Two tutorials play — Magic Junctioning and Gunblade operation. Speak to one of the Garden Faculties and choose a time limit; **10 minutes** is more than sufficient. The cavern is four screens long with a linear path. All enemies inside are a fixed **Lv 5** (they do not scale). A **Fire draw point** sits on the fourth screen. At the far end, Ifrit awaits.`,

    `{{BOSS:Ifrit}}`,

    `Strategy: Ifrit is fixed at Lv 6 and does not scale with the party. He alternates between casting Fire on a single character and using Jump Punch for roughly 100 physical damage. His elemental alignment means he **absorbs Fire** and is **weak to Ice** — Shiva is the correct GF here. Have Quistis summon Shiva while Squall attacks physically each turn. If either character drops below 100 HP, use the Draw command to pull Cure directly from Ifrit and cast it immediately rather than spending an item. Limit Breaks deal more damage than either Shiva or normal attacks, so use them the moment Crisis Level allows. Ifrit also carries **Scan** in his draw list — draw it before finishing the fight as an early junction stock. On defeating him he drops **3–6 G-Returner** items (which revive a KO'd GF) at 100% probability, plus the Ifrit Card. There is no need to reset for a better drop count — three is sufficient.`,

    `**SeeD Rank tip**: When the battle ends and the naming screen appears for Ifrit, let the timer run down as low as possible before confirming — the lower the remaining time, the higher the Judgment score. On original PSX, leaving the screen idle naturally maximizes this. **PC/Remaster note**: confirm the name when the in-battle clock reads 0:01 (not 0:00); confirming at exactly 0:00 registers as 0 remaining and yields the lowest Judgment score on these versions.`,

    `After Ifrit is defeated and joins the party, junction him immediately. Backtrack out of the cavern. Quistis delivers the GF Tutorial on the return trip, then leaves the party.`,

    `**General shops — Balamb, Dollet, Timber, Deling City, Winhill, FH**: All standard item shops in these towns share the same inventory: Potion, Hi-Potion, Phoenix Down, Antidote, Eye Drops, Soft, Echo Screen, Holy Water, Remedy, Tent, Fuel, Normal Ammo, Shotgun Ammo, G-Potion, and G-Returner. With Tonberry's **Familiar** ability active, G-Hi-Potion also becomes available at all shops. Stock **Fuel** early for Balamb Garden navigation later in Disc 1. **Remedy** clears Poison, Darkness, Silence, Petrify, Sleep, Slow, and Curse simultaneously and is the most cost-efficient broad status cure available before endgame items. The **Esthar shop** (Disc 3+) carries a distinct inventory — it omits Antidote, Echo Screen, Holy Water, and G-Potion but adds Dark Ammo, Fire Ammo, and G-Hi-Potion (no Familiar needed), plus Demolition Ammo and Fast Ammo with Familiar.`,
  ],
...
**Achievement: First Salary** — earned automatically the first time Squall receives a SeeD salary payment. This triggers on its own while walking through Balamb Garden after the Training Center events (salary is paid every 24,575 steps). No action is required; simply continue playing.`,
    `{{ENCOUNTERS:0}}`,
...
**Missable — Girl Next Door Magazine**: Pick up the **"Girl Next Door" magazine** from the Timber Maniacs building now. It is needed on the White SeeD Ship (Disc 3) to obtain the Shiva Card from Zone. If it is not collected before Disc 4, it is permanently gone. In the building on the left side of the square, the **old man's house** offers either a **free party heal** OR **500 Gil** — these are mutually exclusive. Talking to the old man first triggers the free heal. Examining the cupboards first locks out the heal and gives 500 Gil instead. If you need the party healed here, speak to the old man before touching anything else. In the **Pub**, challenge the soldiers at the square table to win a **Buel Card**; speak to the drunk at the counter — a dialogue choice determines which card he offers: select **"Buy him a drink"** to receive a **Forbidden Card**, or **"Tell him about the card"** to receive a **Tonberry Card**. The Forbidden Card is generally more useful for refinement. The **"Girl Next Door" magazine** is available in the Timber Maniacs building — pick it up now. It is needed later on the White SeeD Ship and becomes permanently unavailable once Disc 3 ends.`,
  ...
  'd1-dollet-exploration': [
    ...
    **Abolishing Random and Elemental from Dollet** (requires the Queen of Cards to be in Dollet and Galbadian rules carried from Timber's gate guard): **Original/PSX method** — Save in the Dollet Hotel, do a hard console reset (power cycle, not soft reset), then walk outside to the town square and immediately back inside the hotel. Challenge the hotel girl twice but decline both times. On the third challenge, accept and immediately quit. If Open had not yet spread to Dollet, it spreads now. If Open was already active, Random is abolished. To abolish Elemental next: beat the pub owner to access his private room, carry Galbadian rules again (challenge Timber's left gate guard until he stops asking to mix), save in the Dollet Pub (requires Siren's Move-Find ability — the save point is hidden), hard reset, load the pub save, enter the private room, examine the magazine stack near the exit exactly **twice**, then challenge the pub owner and accept then immediately quit the game. This abolishes Elemental. If the Queen is not in Dollet, examine the hotel girl or magazine stack **one extra time** in the respective method. **Remastered/PC method** — In the Remastered version, the card-rule RNG seed is determined by the number of screen transitions that have occurred since the last save-load, rather than by console resets. Hard resets are unnecessary and ineffective. Instead: save outside Dollet and reload. Count screen transitions precisely — entering Dollet counts as one, crossing into the fountain square as another, walking up to the next area as another, and returning back down as another. After accumulating roughly **6 screen transitions** since loading (the exact count that works has been confirmed by community testing, though results can vary slightly between saves and whether the Queen is present), enter the Dollet Hotel and challenge the girl. Accept the game, then immediately exit the Triple Triad screen without playing a card. This has a strong chance of abolishing Random. If it does not work, reload and try varying the screen-transition count by one or two — some saves respond best at 4–7 transitions. To abolish Elemental via the pub: use the same save-load approach, count transitions to reach the pub back room, examine the magazine stack the appropriate number of times, then quit the game screen. Once both rules are gone, only Open and Same/Diff remain in Dollet, making card play far more manageable. **Balamb all-rules abolisher**: There is a man at the **Balamb Town docks** who, when challenged to a card game, will abolish ALL active rules in the Balamb region — including Open. This is unique to Balamb only. If Balamb's card rules have become cluttered with spread rules, visit him to clean the slate. Be aware this also removes Open from Balamb, which then must be re-spread if needed.`,
    ...
    `**Missable — Pet Nametag (Timber)**: Speak to the man at **Timber Square** (near the Timber Maniacs building) and choose **"No way"** when he talks about giving up journalism. This conversation must happen **before finishing Disc 1**; after Disc 2 starts, the trigger closes permanently. On Disc 2, after the Missile Base mission is complete, find him again near the Timber Maniacs building entrance — he gives Squall a **Pet Nametag** as thanks for the encouragement. This is the second of only two Pet Nametags in the game; the other is in D-District Prison Floor 2. If this Disc 1 exchange is skipped, the Pet Nametag is permanently unobtainable.`,
  ...
  'd1-galbadia-garden': [
    ...
    `**To obtain the GF Brothers**: From the sword screen, navigate Right → Up → Right → Up → Right → Up to encounter **Sacred** (it flees quickly). Pull the lever on the right to release the waterflow. Navigate to the machine on the left to lower the bridge. Return to the entrance and proceed all the way up to fight both bosses.`,
    ...
    `**Missable — Location Displayer**: Before handing the student ID to the guard, speak to him and choose to **purchase the Location Displayer for 5,000 Gil**. Once the ID is given, this item is permanently gone. The Location Displayer upgrades the Battle Meter with area-tracking functionality.`,
...
**Missable — Rinoa Card**: The Rinoa Card is permanently unavailable if Caraway's window closes — he only plays it after receiving the Ifrit Card, and the window is open during Disc 1 (this visit) and again briefly on Disc 2 after the Missile Base mission. After that, the card is gone forever.`,
  ],
...
- **Disc 3 bonus**: After completing the **Shumi Village stone quest** and then visiting Shumi Village on Disc 3 to see the follow-up workshop scene, a **Full-Life draw point** appears at the spot where the Master Fisherman used to sit (the lower catwalk of the FH exterior). This is one of the few refillable Full-Life draw points in the game — return to FH on Disc 3 to collect it.`,
  ...
  'd2-exploring-the-world': [
    ...
    `**Balamb under Galbadian occupation**: Zell must be in the party. Speak to the hotel owner and a woman near the entrance for clues. Talk to the guard, walk away, and re-approach. Head to **Zell's House** and enter the room on the right. The **Pandemona Card** can be won from a neighbor girl before liberating the town.`,
  ...
  'd2-return-to-balamb': [
    ...
    `**SeeD rank boost**: Before triggering the harbor confrontation, follow the dog in Balamb. The dog runs from the harbor to the train station to the town square — following it and interacting at the right moment grants **+1 SeeD rank**.`,

    `**Queen of Cards quest step**: After Balamb is liberated, take the Pandemona Card that was just drawn and also have the **Sacred Card** and **MiniMog Card** in hand. Go to Balamb's train station and challenge the Queen of Cards — lose intentionally with both Sacred and MiniMog in the playing hand. She will take both cards and announce her next destination; confirm she moves to **Dollet** (reset if she goes elsewhere). Losing Sacred creates the **Irvine Card** (held by Flo at FH Mayor's house); losing MiniMog creates the **Kiros Card** (held by a man in black in Deling City's shopping arcade). The Balamb Hotel owner now also holds the **Pandemona Card** — challenge him to win it.`,
  ],
...
Once inside the Lunatic Pandora, collect everything Laguna's dream unlocked:\n- At the **base of elevator 3** (before ascending), check for a **Confuse draw point** — stock up here\n- Exit north, take **elevator 3**, exit southeast (down hard-to-see stairs), climb the ladder, check the **east dead-end** for **LuvLuvG** (+20 GF compatibility with all GFs)\n- Right door: **Power Generator** (teaches GF a powerful ability)\n- Middle door: **Silence draw point** (stock up)\n- Left door: **Phoenix Pinion** (teaches Phoenix to a GF — see GF notes below)\n- Check ground near right wall: **Combat King 005** (5/5) if not already obtained\n- While walking north from the elevator, examine the **hole in the left wall** to find the **Spd-J Scroll** (teaches any GF Speed Junction)\n- **Ultima draw point** (accessible only if the Old Key was picked up during the Laguna Pandora dream): unlocked by the key inserted during Disc 1. This Ultima draw point is **one-time only** and does not recharge — stock the full 100 on the first visit. The location is through the doorway unlocked by the Old Key found near the Confuse draw point during the first dream.\n- **Meteor draw point**: accessible at the main entrance of the Pandora on first contact — stock before going deeper.`,
  ...
  'd3-ragnarok': [
    ...
    `**Jumbo Cactuar** (Cactuar Island, small desert island east of the Centra Kashkabald Desert): Find the island east of the Centra Kashkabald Desert and run until the encounter triggers.`,
...
- **Eva% note**: Eva% is Speed-dependent, not purely from the Eva-J junction. Speed provides a base Eva% (e.g., Speed 100 ≈ 9–10% base), and Eva-J adds to this formula — the junction value does not simply replace the base. The **maximum achievable Eva% is 48%**, requiring both 255 Speed *and* 100× Ultima junctioned to Eva-J simultaneously. Neither alone reaches the cap. Haste (50), Triple (40), and Tornado (32) are the best Eva-J spells for most builds.`,
    ...
    `**Permanent stat-up refinement chains** (per stat, all require the GF abilities listed above):
- **HP Up**: Buy 100× Giant's Ring from Esthar Pet Shop → GF Abl Med-RF (Eden) → 10× Gaea's Ring → Forbid Med-RF (Doomtrain) → 10× HP Up. Costs roughly 1.5 million Gil for a full 100× run; use the Tent/Cottage/Mega-Potion loop first.
...
- Weakness: Wind ×1.5`,
  ...
  'd4-the-awakening': [
    ...
    `**Navigating to the Ragnarok**: From Edea's House, head right along the cliff to reach the world map portal chain. Three portals are immediately visible; a fourth is activated by exploration:
...
8. **Save** — unlock last (the final in-castle save point is near the end anyway)`,
  ...
  'd4-ultimecia-castle': [
    ...
    `**First guardian — Sphinxaur/Sphinxara** (Main Hall, top of stairs):`,
...
8. **Repeat from Meteor**`,
    ...
    `**CC Group on the Ragnarok** (if any rare cards were missed): The Left Diamond girl plays rare cards that were modded (following the Queen of Cards chain to Shumi Village). Challenge her repeatedly to recover missed GF cards. The PuPu Card is never played by any CC member.`,
  ],
  'd4-final-preparations': [
    `**Perfect Game — six completion levels**: A "Perfect Game" means having achieved all requirements for a given level. Every level inherits the prior level's conditions.\n\n**Level 1** (Foundation): Able to beat the main game; able to beat Ultima Weapon and Omega Weapon (keeping Ultima alive is recommended for the fixed Deep Sea Research Center encounters — see below); best weapons for all six main playable characters; all Limit Breaks unlocked (skip Angelo Recover and Angelo Reverse on Rinoa until Angelo Search farming is complete — those two trigger automatically and interrupt Angelo Search); all 16 GFs obtained; at least 1 of every card; all 32 magic slots filled for every character; all side quests completed except Ultima Weapon; Battle Meter with all upgrades; Max SeeD Rank A; either zero or all 30 written SeeD tests answered correctly; X-ATM092 destroyed rather than escaped from (destroying it keeps the Dollet car intact and grants a bonus to the starting SeeD score); **0 total kills for Seifer and 0 for Edea** throughout the entire game.\n\n**Level 2**: adds: at least 1 of each item in the game; zero KOs for all party members across the entire playthrough; only 1 escape from battle ever used (the mandatory X-ATM092 run on the Dollet beach); all 6 main characters and all 16 GFs at Level 100; max compatibility on all GFs for all characters; all treasures collected; max stats via junctions; all Tutorial entries unlocked; only Diff and Open card rules active in any region; all GFs retaining their unique (non-restorable) abilities.\n\n**Level 3**: adds: 100 copies of every card except PuPu and the Lv8/9/10 rare cards, which are capped at exactly 1 each; base Str/Vit/Mag/Spr maxed for all characters without any junctions equipped; 100 of nearly every item (exceptions: stat-boosting items, elemental attack/guard, status attack/guard, Dark Matter, Shaman Stone, Hero Trial, and Pocketstation-exclusive items); all GFs at 9,999 HP.\n\n**Level 4**: adds: max Gil (999,999); max kill count (~65,535) for all 6 main characters; 100 of every item including stat-boosting items; max Speed for all characters.\n\n**Level 5**: adds: max Luck for all characters.\n\n**Level 6 (Pocketstation hardware required)**: adds: 100 of every Pocketstation-exclusive item (Ribbon, Mog's Amulet, Friendship, Chocobo's Tag, additional Pet Nametags); Boko the Chocobo GF powered up via the Chocobo World minigame to deal 9,999+ damage.\n\n**Why 0 kills for Seifer and Edea**: The game's kill counter caps at 65,535 per character. Reaching that count for a temporary character like Seifer alone would require over 300 hours of grinding — purely impractical. More meaningfully, keeping their counts at zero demonstrates that the main party never needed them, which is a point of pride rather than a concession. The zero is also cleaner than any partial number. Both characters must fight, but the permanent party members deliver every finishing blow.\n\n**Card collection target (Level 3)**: The PuPu Card is the only card that cannot be re-obtained once lost — never Card Mod it. All Lv8, Lv9, and Lv10 rare cards are limited to 1 copy each, but can be won back from the CC Group Left Diamond girl on the Ragnarok (Disc 4). For a Level 3+ run, hold all rare cards and target 100 copies of every common card via card games and AP grinding.`,

    `**Stat-maxing on Disc 4 — prerequisite checklist**: For players pursuing maximum stats, the following must be true before arriving here or it may be too late. (1) **CC Group all defeated by end of Disc 3** — they need to be aboard the Ragnarok on Disc 4 for the Left Diamond girl to recover modded rare cards and for the Gilgamesh/Eden card sources. (2) **All Esthar shops physically visited** at least once during Disc 3 — each shop must be entered at least once before Call Shop lists it; if any shop (especially Esthar Pet Shop and Esthar Shop!!!) was skipped, it is permanently unavailable via Call Shop on Disc 4. (3) **Ultima Weapon still alive** in the Deep Sea Research Center — the Tri-Face fixed encounters are the only reliable Curse Spike source for Luck Up refinement; defeating Ultima Weapon permanently removes them. (4) **Siren at Lv 100** if Luck Up refinement is planned — the Dark Matter recipe (100× Curse Spike → 1 Dark Matter) requires Siren to be at level 100 for Tool-RF to access it. (5) Required GF abilities active: Quezacotl Card Mod, Siren Tool-RF, Carbuncle Recov Med-RF, Doomtrain Forbid Med-RF, Tonberry Haggle/Sell-High/Familiar/Call Shop, Eden GF Abl Med-RF. All of these must be learned before any items in the stat-up chains can be acquired efficiently.`,
...
- Draw: Lv 1-65: Apocalypse`,
    ...
    `Strategy: **Critical warnings**: (1) If a KO'd character is left down too long, Ultimecia uses **"absorbed into time"** to permanently remove that character from the battle — revive KO'd members immediately. (2) Both Griever and Ultimecia Final Form can **blow away randomly selected magic from any character's stock** — if junctioned magic is removed, that junction stat drops to its base value. Defeat each form quickly to limit this danger. (3) Do not summon GFs during the Ultimecia/Griever or Ultimecia Final Form phases — those forms use a **GF lethal strike** counter that can permanently KO summoned GFs.
Full sequence:
...
export const CHAPTER_ENCOUNTERS = {
  'd1-preparing-for-the-exam': [
    ...
    },
    {
      ...
      enemies: [
        { id: "enemy-bomb", name: "Bomb", notes: "Draws Fire, Fira, Firaga, Meltdown; mug Bomb Fragment; card  →  Bomb card", lvMin: 5, lvMax: 5 },
        { id: "enemy-buel", name: "Buel", notes: "Draws Fire, Thunder, Blizzard (full elemental trio at low levels)", lvMin: 5, lvMax: 5 },
      ],
    },
  ],
  'd1-the-seed-exam': [
...
**Physical Damage Formula**: Used by all physical attacks including Squall's gunblade strikes and physical-type GF summons.
...
**Magical Damage Formula**: Applied to offensive magic spells and magic-type GF summons.
...
**Crit Chance and Bonus Damage**: A critical hit doubles the final damage total.
...
**ATB Fill Rate**: How quickly each character's Active Time Battle bar fills is governed by Speed (Spd stat).
...
**EXP and AP — how battle endings change rewards**: The method used to end a battle determines whether EXP, AP, and item drops are awarded.
...
**Enemy Level Scaling**: Enemy HP, Str, Vit, Mag, Spr, and Spd all scale to a range centered on the party's average level.
...
**Item Drop Probability**: At battle end, each enemy checks four independent drop slots in sequence.
...
**Harmful: Poison**: Drains HP equal to 1/16 of maximum HP per tick, persisting between battles until cured. Not immediately lethal but compounds over long dungeons with no healing.
- Remove with: Antidote, Esuna, Remedy, Panacea
- Prevent via: Antidote or Esuna at ST-Def-J

**Harmful: Sleep**: Target is unable to act; ATB is frozen and Eva drops to 0%. Any incoming physical hit wakes the target immediately.
...
**Harmful: Silence**: Prevents magic casting, GF summons, and the Draw command; physical attacks are unaffected. A silenced character cannot draw spells from enemies.
...
**Harmful: Blind (Darkness)**: Reduces physical hit accuracy to near zero. Has no effect on magic accuracy.
- Remove with: Eye Drops, Esuna, Remedy
- Excellent to inflict on physical-heavy enemies via the Blind spell or Dark Eye item

**Harmful: Petrify (Countdown)**: A stone countdown (about 20 seconds) ticks down; if it reaches zero the target is fully petrified. Contact while fully petrified can shatter the target. **Petrified enemies yield zero EXP, AP, and drop no items** — avoid unintentionally petrifying enemies when farming.
...
**Harmful: Slow**: Halves the rate at which the target's ATB gauge fills, effectively cutting their turn frequency in half.
- Remove with: Haste, Esuna, Dispel, Remedy
- Pairing Slow with Doom is a reliable strategy against many bosses

**Harmful: Stop**: Completely freezes the target's ATB gauge; Eva drops to 0% and no actions can be taken until the effect ends. Unlike Slow, Stop also halts any active status duration timers on the target.
...
**Harmful: Confuse**: Forces the target to take random actions, potentially attacking allies. Damage to the confused unit may clear the status.
...
**Harmful: Berserk**: Forces the target to use only physical attacks each turn, with attack power boosted by 50%, but completely out of the player's control. Does not persist after battle.
...
**Harmful: Doom**: Starts a counter at **8** that counts down over **16 seconds** (at default Battle Speed). When it reaches zero the target is KO'd instantly regardless of HP. The counter is time-based, so faster Battle Speed settings reduce available response time. Casting Zombie on a Doomed target also cancels the countdown.
...
**Harmful: Curse**: Prevents Limit Breaks from triggering, even at Crisis Level 4, and blocks Aura from enabling them.
...
**Harmful: Zombie**: Reverses healing: Cure spells deal damage, Full-Life kills instantly. **Holy deals double damage** to zombified/undead targets. Fire deals normal damage. Life and Full-Life instantly KO zombified enemies instead of reviving them.
...
**Harmful: Vit 0 (Meltdown effect)**: Reduces both Vitality and Spirit to zero simultaneously — maximum damage from physical and magic attacks. Clears after battle. **No enemy in the game is immune to Vit 0**, making Meltdown universally applicable.
...
**Beneficial: Float**: Target hovers above ground — all earth-based attacks miss entirely. Dispel removes it. Auto-Float can be junctioned or granted by Mighty Guard. Self-inflict before Quake-heavy encounters.

**Harmful: KO (Death)**: Target collapses and cannot act; must be revived.
...
**Beneficial: Protect**: Halves all incoming physical damage. Stacks multiplicatively with Vit-J defensive bonuses.
...
**Beneficial: Shell**: Halves all incoming magical damage. Stacks multiplicatively with Spr-J.
...
**Beneficial: Reflect**: Bounces single-target magic back at the caster.
...
**Beneficial: Haste**: Doubles the ATB gauge fill rate — the single most impactful buff in the game.
...
**Beneficial: Regen**: Restores approximately 5% of max HP per tick at regular intervals.
...
**Beneficial: Aura**: Enables Limit Breaks to appear regardless of current HP (normally limited to low HP or Crisis Level). Lasts until battle ends or dispelled.
...
**Beneficial: Double**: Allows the target to cast any spell twice per action, consuming 2 stock per use.
- Sources: Double spell, Cerberus GF
- Double + Ultima is a high-efficiency offensive combo for magic builds

**Beneficial: Triple**: Allows the target to cast any spell three times per action, consuming 3 stock per use.
...
**Harmful: The End**: A unique status from Selphie's Limit Break that peacefully removes any non-undead enemy from battle. Undead enemies and bosses are immune.
...
**Beneficial: Defend**: Completely negates all incoming physical damage and halves magic damage for that turn. The status lasts only until the character's next action.
...
**HP-J (Health Points)**: Raises maximum HP — the most critical survivability junction. Stack with HP+% GF abilities to reach the 9,999 cap.
...
**Str-J**: Increases physical attack power. Priority junction for Squall, Zell, and Irvine.
...
**Vit-J**: Raises physical defense, reducing incoming physical damage.
...
**Mag-J**: Increases offensive spell damage and healing spell potency.
...
**Spr-J**: Raises magical defense, reducing damage from spells and GF summons.
- Best: Ultima (+95 Spr), Full-Life (+85 Spr), Reflect (+72 Spr), Curaga (+65 Spr)

**Spd-J**: Increases ATB fill rate — the most impactful stat for turn frequency.
...
**Eva-J**: Increases the chance to dodge physical attacks entirely.
- Best: Triple (+40 Eva), Tornado (+32 Eva)

**Hit-J**: Increases physical attack accuracy and status-infliction success rate.
...
**Luck-J**: Affects critical hit rate, item drop rates, card game outcomes, and certain random events.
...
**ST-Atk-J (Status Attack)**: Gives every physical hit a chance to inflict the junctioned status on the target.
...
**ST-Def-J (Status Defense)**: Grants immunity or strong resistance to the junctioned status at 100 stock.
...
**Elem-Atk-J (Elemental Attack)**: Adds an elemental affinity to physical strikes, amplifying damage against enemies with a matching weakness.
...
**Elem-Def-J (Elemental Defense)**: Reduces or nullifies damage from the junctioned element.
...
- **Character abilities**: Auto-Haste + Str+60% + Auto-Shell + Spr+60% on all three`,
  },
  {
    ...
    disc: 0,
    content: `Squall holds a SeeD rank that determines the salary paid automatically at regular intervals as the player walks (payment is step-based, not time-based). Starting rank is earned through field exam performance. Rank can rise or fall during the playthrough.

**SeeD Experience system**: The underlying metric is **SeeD Experience** (abbreviated SeeD Exp). SeeD Rank = ⌊SeeD Exp ÷ 100⌋. Rank A is the maximum, requiring 3,100 SeeD Exp. The absolute cap is 3,110 SeeD Exp — only 10 points above the Rank A threshold, which is why receiving a salary at Rank A drops the rank to 30 (salary costs 10 SeeD Exp). To prevent dropping from Rank A, kill at least 10 enemies before the next salary payment.

**SeeD Exp gains and losses**:
- Each enemy killed: **+1 SeeD Exp** (GF kills do not count)
- Salary paid: **−10 SeeD Exp**
- Passing a SeeD written test: **rounds SeeD Exp up to the next 100** (e.g., 2,350 → 2,400, raising rank to 24)
- Tests can be taken up to the rank matching Squall's character level

**Salary payment interval**: Salary is paid every **24,575 steps**. One visual step is approximately 3–5 internal steps; riding a Chocobo on the world map counts ~7–8 internal steps per visual move. Driving a car also counts. Moving Balamb Garden or the Ragnarok does NOT count as steps.

**Initial SeeD Rank** (earned during the SeeD Exam): Starting rank is calculated from five exam categories plus a bonus.
- **Conduct** (time left to reach shore): 25:00+ → +80; 24:59–23:00 → +70/+60; 22:59–20:00 → +50; lower times reduce or penalize
- **Judgment** (time left on Ifrit fight timer): 0:00–0:07 → +100; 0:08–0:29 → +80; 0:30–0:59 → +70; 1:00–1:59 → +50; let the naming screen sit idle to get maximum judgment time
- **Attack** (enemy kills during exam): 75+ kills → +100; 50–74 → +75; 25–49 → +50; 20–24 → +50; 15–19 → +10; 10–14 → −50; 0–9 → −100
- **Spirit** (number of escapes): 1 escape → +100; 2 → +50; 3–4 → 0; 5–9 → −50; 10+ → −100
- **Attitude** (deductions for specific behaviors): Full points with 0 deductions (+100); deductions from: talking to most NPCs while in SeeD uniform (−1 each), disregarding Seifer's submarine order (−1), moving toward tower before ordered (−1), talking to Biggs after defeating him (−2), jumping off Selphie's cliff (−5), failing to talk to the dog before X-ATM092 reaches it (−10), hiding in the café during X-ATM092 (−20)
- **Bonus**: destroying X-ATM092 adds **+1 entire SeeD Rank** to the calculated result
- **Formula**: Initial Rank = ⌊(500 + sum of all modifiers) ÷ 100⌋ + Bonus (minimum 1, maximum 10)

**In-game SeeD Exp events** (10 events across the full game that directly adjust SeeD Exp):
1. Showing off the Revolver gunblade to students on Balamb Garden 2F: **−1 SeeD Rank**
2. Trying to sneak out during the SeeD graduation night (3+ attempts): **−1 SeeD Rank**
3. Timber Train Mission — detected 0 times: **+1 Rank**; detected 1–9 times: no change; 10+ times: **−1 Rank**
4. Escaping from the Tomb of the Unknown King via the map option: **−1 SeeD Rank**
5. Missile Base Mission — perfect (right info, disguise route, 10-minute timer): full +200 SeeD Exp; penalties reduce this
6. After the NORG fight, a male student outside the second-floor classroom asks Squall to show him magic — complying (casting any spell when prompted) costs **−1 SeeD Rank**
7. Master Fisherman's Quest completion at FH: **+20 SeeD Exp**
8. Finding the Captain in Balamb — using the dog: **+100 SeeD Exp**; boarding the train: **+65 SeeD Exp**
9. Shumi Village stone-collecting quest (Phase 1): **+50 SeeD Exp**
10. Defeating Ultima Weapon in the Deep Sea Research Center: **+1 SeeD Rank**

**SeeD Salary Table (paid per step interval)**:
- Rank 1: 500 Gil | Rank 2: 1,000 | Rank 3: 1,500 | Rank 4: 2,000 | Rank 5: 3,000
- Rank 6: 4,000 | Rank 7: 5,000 | Rank 8: 6,000 | Rank 9: 7,000 | Rank 10: 8,000
- Rank 11: 9,000 | Rank 12: 10,000 | Rank 13: 11,000 | Rank 14: 12,000 | Rank 15: 12,500
- Rank 16: 13,000 | Rank 17: 13,500 | Rank 18: 14,000 | Rank 19: 14,500 | Rank 20: 15,000
- Rank 21: 15,500 | Rank 22: 16,000 | Rank 23: 16,500 | Rank 24: 17,000 | Rank 25: 17,500
- Rank 26: 18,000 | Rank 27: 18,500 | Rank 28: 19,000 | Rank 29: 19,500 | Rank 30: 20,000
- Rank A: 30,000 Gil (maximum rank)

**SeeD Test Answers — Tests 01–05**:
- Test 01 (Q1–Q10): Y · N · Y · Y · Y · N · N · Y · N · N
- Test 02 (Q1–Q10): Y · N · Y · Y · Y · N · Y · Y · N · N
- Test 03 (Q1–Q10): N · N · Y · N · Y · Y · Y · N · Y · N
- Test 04 (Q1–Q10): N · Y · Y · Y · N · N · Y · Y · N · N
- Test 05 (Q1–Q10): N · N · N · Y · Y · N · N · Y · Y · Y

**SeeD Test Answers — Tests 06–10**:
- Test 06 (Q1–Q10): Y · N · Y · Y · N · N · Y · Y · N · Y
- Test 07 (Q1–Q10): Y · Y · Y · Y · Y · Y · N · Y · Y · N
- Test 08 (Q1–Q10): N · Y · N · N · Y · Y · N · N · Y · N
- Test 09 (Q1–Q10): N · Y · N · N · N · N · N · N · Y · Y
- Test 10 (Q1–Q10): Y · N · N · N · N · N · N · N · Y · N

**SeeD Test Answers — Tests 11–15**:
- Test 11 (Q1–Q10): Y · Y · N · Y · Y · N · Y · N · N · Y
- Test 12 (Q1–Q10): N · Y · N · N · Y · N · Y · N · Y · N
- Test 13 (Q1–Q10): Y · N · N · N · Y · N · N · N · N · N
- Test 14 (Q1–Q10): Y · Y · Y · Y · N · Y · Y · N · Y · N
- Test 15 (Q1–Q10): Y · Y · N · N · N · N · N · Y · N · Y

**SeeD Test Answers — Tests 16–20**:
- Test 16 (Q1–Q10): Y · N · N · Y · N · Y · N · N · Y · N
- Test 17 (Q1–Q10): Y · N · N · N · Y · N · N · Y · N · N
- Test 18 (Q1–Q10): Y · N · N · N · Y · N · N · N · N · N
- Test 19 (Q1–Q10): Y · N · N · Y · N · N · N · N · N · Y
- Test 20 (Q1–Q10): Y · Y · N · Y · N · Y · Y · Y · N · N

**SeeD Test Answers — Tests 21–25**:
- Test 21 (Q1–Q10): Y · Y · Y · Y · N · N · Y · Y · Y · N
- Test 22 (Q1–Q10): N · N · N · Y · N · N · N · Y · Y · N
- Test 23 (Q1–Q10): Y · N · N · N · N · Y · Y · Y · Y · Y
- Test 24 (Q1–Q10): Y · Y · N · N · Y · Y · N · N · N · Y
- Test 25 (Q1–Q10): Y · N · Y · Y · Y · N · N · Y · N · N

**SeeD Test Answers — Tests 26–30**:
- Test 26 (Q1–Q10): Y · Y · N · Y · N · Y · N · Y · N · N
- Test 27 (Q1–Q10): N · Y · N · N · N · N · Y · N · Y · N
- Test 28 (Q1–Q10): Y · N · N · Y · Y · Y · N · Y · N · N
- Test 29 (Q1–Q10): N · N · N · Y · Y · N · N · N · Y · N
- Test 30 (Q1–Q10): N · Y · N · N · N · N · Y · N · N · N`,
  },
  {
...
**Play Rule: Open**: Both players can see all cards in each other's hand before placement begins. Eliminates guesswork entirely — play becomes a pure strategy game with full information.

**Play Rule: Same**: When the placed card's values match two or more sides touching adjacent opponent cards simultaneously, all matching opponent cards flip at once. Chains with Combo.
...
**Play Rule: Same Wall**: Treats the outer grid border as a value of A (10) for the purpose of the Same rule. Allows edge placements to trigger Same even without two adjacent opponent cards.

**Play Rule: Plus**: When the placed card's value plus the touching side of one opponent card equals the same sum as the placed card's value plus a second adjacent touching side, all involved opponent cards flip simultaneously.
...
**Play Rule: Combo**: Cards that flip due to Same, Plus, or Same Wall then act as though freshly placed, potentially flipping additional adjacent opponent cards through normal value comparisons. Can cascade across the entire board in a single turn.

**Play Rule: Elemental**: Random elemental symbols are assigned to some grid squares before the game. A card placed on a matching element gains +1 to all four values; a mismatched element imposes −1 to all four. Elements correspond to the eight GF affinities (Fire, Ice, Thunder, Earth, Wind, Water, Holy, Poison).

**Play Rule: Sudden Death**: If the final score is tied at 5–5, both players keep only the cards currently showing their color, then replay with those cards. Continues until a winner emerges.
...
**Play Rule: Random**: Instead of choosing five cards, both players have five cards dealt randomly from their entire deck.
- Mitigate by keeping your full deck stocked with high-value cards; low-value cards in the collection become a liability

**Trade Rule — One**: Winner takes one card from the loser's hand. The most common rule; standard for most NPC opponents.

**Trade Rule — Diff**: Winner takes a number of cards equal to the score difference. Winning 7–3 earns four cards; winning 8–2 earns six. Most rewarding for dominant victories.

**Trade Rule — Direct**: Each player claims only the cards that display their color at game end, regardless of which player originally played them. Your opponent can never take a card off your side of the board.

**Trade Rule — All**: Winner takes the opponent's entire five-card hand. High stakes for both sides — only play All against easy opponents when your own deck contains no irreplaceable cards.

**Regional Starting Rules** (default on a fresh playthrough):
- **Balamb**: Open only
...
**Abolishing Rules**: To remove an unwanted rule from a region, challenge someone there with mixed rules (from a different region) and hope for abolition — save before attempting. Each attempt has a random chance of abolishing a rule, adding a rule, or no change.
...
**Summoning**: Selecting the GF command begins a charge animation that fills a bar over several seconds. During the entire charging phase, the junctioning character absorbs incoming damage on the GF's behalf using the GF's own HP pool.
...
**Boost**: While a GF is charging, pressing Square repeatedly increases the final damage multiplier.
...
**GF Compatibility**: Each GF has a separate Compatibility value with every character, ranging from 0 to 1,000. Higher Compatibility reduces the summon charge time for that GF with that character.
...
**Compatibility-Raising Items**: Using certain consumable items in the menu raises a GF's Compatibility with the current character. Most specific items give **+3.2** to the target GF but **−1.6** to every other junctioned GF. Half-value items (Arctic Wind, Bomb Fragment, Poison Powder, Shear Feather) give +1.6/−0.8 instead. Fish Fin is a further exception at +2/−1. **LuvLuv G** is the only item with no penalty — it raises every junctionable GF by +20 simultaneously and is the fastest single method to max all compatibilities. Values per use:
...
**AP Spending Priorities**: Each GF unlocks abilities from a branching tree using battle-earned AP. Recommended early targets:
...
**Rival Pairs (Compatibility Penalty)**: Summoning any GF gives that GF +20 Compatibility, costs all other junctioned GFs −1.6, and costs the opposed (rival) GF an additional −10. Complete rival pairs:
...
**GF leveling — EXP per level**: Most GFs need **500 EXP** to gain a level. Three GFs level faster: **Siren**, **Carbuncle**, and **Cerberus** each require only **400 EXP per level**. **Eden** levels much slower, requiring **1,000 EXP per level**. All GFs gain EXP from battles as long as they are junctioned to a character who participated in the fight. All GFs cap at level 100.

**Damage cap exceptions**: The standard battle damage cap of 9,999 has three exceptions involving GFs: **Eden's Eternal Breath** can reach **60,000 damage** at maximum Boost; **Lv100 Cactuar's 1000 Needles** deals exactly **10,000 damage** (ignores the 9,999 cap); and **Chocobo's ChocoBocle** attack (requires Chocobo World — built into FF8 Remastered) can also exceed 9,999.

**Eden — Unique Properties**: Eden is the last GF available and has unique properties compared to the others.
