...
export interface SearchResult {
  id: string
  type: 'chapter' | 'achievement' | 'missable' | 'card' | 'gf' | 'refinement' | 'item' | 'weapon' | 'enemy'
  title: string
  subtitle: string
  chapterId?: string
}
...
export function useSearch(data: MasterData) {
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    for (const it of data.lookup.items) {
      if (it.name.toLowerCase().includes(q) || it.obtain.toLowerCase().includes(q)) {
        out.push({
          id: it.id,
          type: 'item',
          title: it.name,
          subtitle: `${it.section}${it.obtain ? ' · ' + it.obtain.slice(0, 45) : ''}`,
        })
      }
    }
